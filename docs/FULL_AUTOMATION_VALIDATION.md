...
| 명령 | 결과 | 확인 범위 |
|---|---:|---|
| `python -m pytest tests apps/srs-spec/tests -q` | `203 passed` | 최종 Root의 보호된 로컬 조건까지 포함한 통합 오케스트레이터, 상태·잠금, SRS `updated` 기간 연계, outbox, 런처, 예약 스크립트와 기존 SRS/이슈 회귀 |
...
격리 worktree에서는 실데이터 선택 테스트 4개가 skip되어 `199 passed, 4 skipped`였고, 최종 Root에서는 보호된 로컬 조건을 사용해 모두 통과했다. 민감하지 않은 E2E 요약은 Git 제외 경로 `.migration/full-automation-smoke/summary.json`에 저장했다.
...
| `python automation.py --check-local` | 0 | 보호된 SRS·이슈·메일 설정과 SRS 런타임 snapshot 경로를 외부 연결 전에 확인 |
| `scripts/install_automation_task.ps1 -Install` | 0 | 작업 등록 후 action/working directory/평일 bitmask 62/09:00/settings 재조회 일치, 상태 `Ready` |
...
- 최종 Root에서 `--check-local`이 통과했고 `ALM_QA_Automation_Daily`를 등록·재조회 검증했다. 다음 예약 시각은 2026-09-22 09:00 KST이며 아직 실제 데이터 실행 이력은 없다.
...
1. 첫 실제 통합 실행의 SRS·이슈 건수, PDF, `dataComplete`, 후보 근거 확인.
2. 받은 편지함에서 실제 Message-ID 한 건 수신과 중복 없음 확인.
3. 성공 manifest를 사용한 레거시 작업 비활성화와 복구 가능성 확인.
4. 장시간 실행, 재부팅 후 StartWhenAvailable, 네트워크 복구, 강제 종료 후 `SENDING` 운영 절차 확인.
