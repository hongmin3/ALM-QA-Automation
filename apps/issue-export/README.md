...
`--check`는 설정 파일·PAT·PDF 엔진과 실제 Polarion 서버 접속 권한을 한 번에
...
| `-y`, `--yes` | 이전 버전 호환 옵션 (현재는 기존 결과를 백업 보존) |
| `--check` | 서버 조회를 포함해 준비 상태 점검 |
| `--check-local` | 서버 접속 없이 설정·PAT 존재·PDF 엔진 점검 |
| `--config PATH` | 설정 파일 경로 (기본값 `config.yaml`) |
| `--help` | 도움말 |

### 이전 결과를 남기고 싶을 때

임시 폴더에서 수집한 뒤 완전 성공한 결과만 지정 출력 폴더로 확정합니다. 기존 결과는 `.previous-<runId>` 폴더에 보존합니다. 부분/실패 결과는 `.failed-<runId>`에 남으며 기존 성공 결과는 교체하지 않습니다. 실행 manifest에는 상태·건수·제한·중복·경고·PDF 결과가 남습니다. 종료 코드는 정상 0, 부분 완료 4, 실패 1입니다.

같은 출력 위치는 잠금으로 동시 실행을 막습니다. 강제 종료 시 남은 staging/previous 폴더는 수동 복구 확인이 필요합니다. 백업 자동 보존 기간 정리는 아직 없습니다.
...
- 실패한 실행은 기존 성공 결과를 대체하지 않고 진단 자료를 보존한다
...
- **v9 (현재)**: 사용 편의성 개선. 이슈 ID만 넣으면 되는 `-id` 옵션(쿼리 이스케이프 자동
  ...
  결과 폴더 백업 보존과 `-o`·`--timestamp`, 진행 상황·남은 시간 표시, 일시적 오류
