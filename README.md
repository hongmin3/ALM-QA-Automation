...
**Polarion ALM의 SRS 사양서와 이슈를 수집하고, 변경 근거와 PDF·HTML·Markdown 산출물로 연결하는 QA 업무 자동화 도구입니다.** 두 개의 독립 자동화를 하나의 Windows 메뉴와 저장소에서 관리합니다.
...
공통 실행기는 두 앱의 실행 경로와 인자를 연결합니다. 수집·렌더링·설정은 각 앱에 유지해 기존 동작을 보존합니다. SRS 결과 메일은 운영 설정에서 활성화한 경우 발송합니다.
...
SRS 예약 실행은 기존 `VXvue_SRS_Spec_Automation` 및 `_CatchUp` 작업을 사용합니다. 작업 디렉터리는 `apps/srs-spec`이며 신규 등록 진입점은 앱의 `scripts/install_task.ps1`입니다. 이슈 내보내기는 수동 실행을 기본으로 합니다.
...
| 후속 계획 | SRS·이슈 통합 예약 수집, 연관 분석, 알림 재시도·중복 방지 |
...
결과는 `.observation/state.json` 및 `runs/<실행 ID>/summary.md`에 저장됩니다. 검토 상태는 `NEW`, `IN_REVIEW`, `DONE`, `EXCLUDED`입니다. 이 폴더는 원문 근거를 포함하므로 Git에서 제외됩니다.
